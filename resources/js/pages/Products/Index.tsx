...
// ── Image thumbnail with view badge ──────────────────────────────────────
function ImageWithBadge({ src, alt, viewCount, size = 'sm' }: { src: string; alt: string; viewCount: number; size?: 'sm' | 'lg' }) {
...
export default function Index() {
    ...
    // Fetch today's per-image view counts for badge display
    const [imageViews, setImageViews] = useState<Record<string, number>>({});
    useEffect(() => {
        if (isAdmin || !myProfile?.id || !myProfile?.subscription) return;
            ...
            <div className="w-full max-w-7xl mx-auto py-6 px-4 lg:px-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-7 mb-6">
                    <div className='w-full flex justify-center flex-col gap-y-5'>
                        ...
                        {/* 2. My Images table — between Stats and Orders */}
